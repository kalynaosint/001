...
// ---- 投票截止：2 小时（毫秒） ----
const VOTE_DURATION_MS = 2 * 60 * 60 * 1000;
...
function computeVoteResult(request, stepKey) {
    ...
    voters.forEach(v => {
        ...
        if (involvedOperatorRole && member.role === involvedOperatorRole) {
    ...
    // 判断时间是否过截止
    const deadline = request.submittedAt + VOTE_DURATION_MS;
    const now = Date.now();
...
function castVote(requestId, stepKey, userId, choice) {
    ...
    // 检查是否过截止
    const now = Date.now();
    if (now >= req.submittedAt + VOTE_DURATION_MS) {
